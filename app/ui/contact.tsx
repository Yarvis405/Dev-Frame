...
const serviceStyle = `p-1 flex gap-1 items-center rounded-md has-[:checked]:bg-purple-300 has-[:checked]:text-white has-[:checked]:font-medium has-[:checked]:scale-105 transition-all active:scale-110`
            ...
            <section id="contact" className="p-4 md:px-[15%] my-4 flex flex-col justify-center items-center gap-2 md:w-full">
                ...
                <form className="flex flex-col gap-6 md:gap-12 w-full" action={url.current} method="POST" target="_blank">
                    ...
                    <div className={inputContainer}>
                        ...
                        <input type="text" max="100" id="client" name="client" alt="Name or Organization" placeholder="Name or Organization" required className={inputStyle} />
                    </div>

                    <div className={inputContainer}>
                        <label htmlFor="email">Email:</label>
                        <input type="email" max="200" id="email" name="email" alt="Type your email" placeholder="user@example.com" required className={inputStyle} />
                    </div>

                    <input type="date" name="date" value={date} title="its suppose to be an automatic value" disabled hidden />
                    ...
                    <input type="submit" value="submit" alt="submit form" className="text-white text-2xl md:text-4xl bg-purple-300 font-medium rounded-md active:bg-purple-400 active:shadow-sm active:scale-105 transition" />
